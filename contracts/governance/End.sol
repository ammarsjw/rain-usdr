...
contract End is IEnd, AccessControl, ReentrancyGuard {
    ...
    function skip(bytes32 ilkId, uint256 auctionId) external nonReentrant {
        ...
        // Mismatch would reinstate the debt at the wrong ilk's rate and corrupt the wrong settlement
        // snapshot.
        if (saleIlkId != ilkId) {
    ...
    function cage(bytes32 ilkId) external {
        ...
        // Halting the auction house: after global settlement the auction price keeps decaying while the
        // settlement price below is fixed forever, so any still-running auction becomes a risk-free arbitrage
        // against USDR redeemers once the curve crosses break-even and collateral bought there leaves the
        // redemption pool permanently. The auction house is global (it serves every collateral type), so the
        // first caged ilk halts it and later cages skip the already-dead house. `yank` is deliberately not
        // live-gated, so `skip` still reclaims in-flight auctions after the halt. Deployments with no auction
        // house configured skip this.
        IDutchAuction dutchAuction = liquidationTrigger.dutchAuction();
