...
import { _PAUSE_FROB, _RAY, _WAD, _WARD_ROLE } from "../shared/Constants.sol";
import {
    FeeRecipientNotSet,
...
contract VaultEngine is IVaultEngine, AccessControl {
    ...
    function file(bytes32 ilkId, bytes32 what, uint256 data) external onlyRole(_WARD_ROLE) {
        ...
        if (what == "spot") {
            ...
            // Safety factor for the dynamic ceiling [wad]. Zero disables the formula and leaves the static
            ...
            // Permanently marks the ilk fee-exempt. One-way: there is deliberately no un-exempt path, so the
            ...
            // Marking requires the ilk to be fee-clean: a rate already above RAY means fees have accrued and
            // the PSM invariant (rate == RAY forever) is already broken for this ilk.
    ...
    function frob(uint256 vaultId, address v, address w, int256 dink, int256 dart) external {
        ...
        // Emergency pause check: when the Governor is wired and the FROB scope is paused, all vault
        // modifications are blocked. Unlike the solvency gate below, this stops risk-decreasing operations
        // too.
        ...
        // Ceiling check: either debt is being repaid (dart decreased), or both the effective ilk ceiling and
        // the global ceiling must hold after the change. Repayments always bypass ceilings so a tightened
        // dynamic line never traps existing positions.
        if (!(dart <= 0 || Math.both(Math.umul(ilk.globalArt, ilk.rate) <= effectiveLine(ilkId), debt <= globalLine))) {
    ...
    function effectiveLine(bytes32 ilkId) public view returns (uint256) {
        ...
        // Use the larger of live and lagged liquidity so growth takes effect immediately while shrinkage is
        // delayed.
        ...
        // Ceiling = liquidity [wad] × f_safety [wad] → wad of USDR, scaled to rad.
        uint256 dynamic = ((liq * safety) / _WAD) * _RAY;

        return dynamic < line_ ? dynamic : line_;
    }

    /**
     * @dev Refreshes the lagged liquidity snapshot used by {effectiveLine}. Growth is applied immediately;
     *      shrinkage only after {_LIQUIDITY_LAG}. Permissionless via {snapshotLiquidity} and invoked whenever
     *      governance files a new liquidity figure.
