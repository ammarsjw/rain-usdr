 ...
 * @dev Every ilk is configured as exactly one of two kinds. A fixed ilk has no oracle lookup and its price is
 *      pinned to $1, with the trust decision living in listing governance. An oracle-backed ilk reads its
 *      price from the single system-wide OSM, set once via `file("oracleSecurityModule")`. The OSM itself
 *      never learns about fixed ilks. Being registered on the OSM is what needs a price lookup means, and
 *      this contract is the single place that routes between the two kinds via the per-ilk `fixed` flag.
 *      `poke` reverts for unconfigured ilks rather than writing a zero spot, and an oracle-backed ilk the OSM
 *      does not serve fails closed to a zero spot.
 */
contract PriceConverter is IPriceConverter, AccessControl {
    ...
    function file(bytes32 what, uint256 data) external onlyRole(_WARD_ROLE) {
        ...
        if (what == "par") {
            ...
            // standard elsewhere (MatBelowOne).
            if (data == 0) {
    ...
    function poke(bytes32 ilkId) external {
        ...
        // The ilk must have been configured: a configured ilk always carries `mat >= RAY` (enforced at file
        // time), so a zero mat means governance never listed it and poke must revert rather than write a junk
        // spot (it would also divide by zero below).
        if (ilk.mat == 0) {
